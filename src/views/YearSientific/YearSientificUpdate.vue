					...
					<div v-for="(item, index) in ProjetAeeay" :key='index'>
						<div class="category" style="margin-top:20px;">
							<el-form-item label="项目类别" prop="projectType">
								...
								<el-button v-if="index!=0" @click="deleteProject(item)" style="float:right;" type="primary">删除项目类别</el-button>
							</el-form-item>	
						</div>
						<div class="information" v-for="(item1, index1) in item.domain" :key='index1'>
							...
							<el-col :span="8">
								<el-form-item label="预估金额">
									<el-input style="width:90%;" v-model.lazy="item1.projectMoney" placeholder="请输入预估金额" @change="changeEvent(item1)"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="8">
								<el-form-item label="开始时间">
									<el-date-picker style="width:100%;margin-left:0px;" v-model="item1.startDate" type="date" placeholder="开始时间" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd">
									</el-date-picker>
								</el-form-item>
							</el-col>
							<el-col :span="8">
								<el-form-item label="结束时间">
									<el-date-picker style="width:100%;margin-left:0px;" v-model="item1.endDate" type="date" placeholder="结束时间" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd">
									</el-date-picker>
								</el-form-item>
							</el-col>
							<el-col :span="8">
								<el-form-item label="负责人">
									<el-input style="width:90%;" v-model="item1.chargePerson" placeholder="请输入负责人"></el-input>
								</el-form-item>
							</el-col>
							<div style="clear: both;"></div>
							<div class="brief">
								<el-form-item label="项目等级">
									<el-select v-model="item1.projectGrade" style="width:100%;" placeholder="请选择项目类别" class="input">
									<el-option v-for="item in projectGrades" :key="item.dictValue" :label="item.label" :value='item.dictValue'>
									</el-option>
									</el-select>
								</el-form-item>
							</div>
							<div class="brief">
								<el-form-item label="项目简介">
									<el-input style="width:100%;margin-left:0px;" v-model="item1.projectDesc" placeholder="请输入项目简介"></el-input>
								</el-form-item>
							</div>
							<div class="table">
								<span>附件</span>
										...
										<template slot-scope="scope">
											<div @click="getM(scope.$index,index1,index)">
												<el-upload id="autoExamUpload" ref="upload" :file-list="fileList" name="articleImage" :headers="handleHeader" :on-exceed="handleExceed" :on-success="handleSuccess" style="display: inline-block;" action="http://61.134.63.231:9090/organizationService/image/uploadImg">
													<el-input slot="trigger" :limit="1"  v-model="scope.row.annexChinaName"/>
												</el-upload>
											</div>
										</template>
									</el-table-column>
							...
							<el-dialog title="添加物资" :visible.sync="Merchandise" width=98%>
								<div class="operating" style="padding:0 30px;">
									...
									<el-input v-model="zxfsearch" style="display: inline-block;width:200px;float:right;borderRight:none;margin-left: 10%;" placeholder="请输入搜索内容" icon="el-icon-search">
									</el-input>
								</div>
								<div class="ztable">
									<el-table :data="item1.tableDatas" ref="tableDatas" tooltip-effect="dark" border stripe style="width: 100%" @selection-change='selectRow'>
										...
										<el-table-column label="计划实施时间" width="230">
											<template slot-scope="scope">
												<el-date-picker v-model="scope.row.startDate" type="date" placeholder="计划交付时间" format="yyyy-MM-dd" value-format="yyyy-MM-dd">
												</el-date-picker>
											</template>
										</el-table-column>
										<el-table-column label="计划交付时间" width="230">
											<template slot-scope="scope">
												<el-date-picker v-model="scope.row.endDate" type="date" placeholder="计划交付时间" format="yyyy-MM-dd" value-format="yyyy-MM-dd">
												</el-date-picker>
											</template>
										</el-table-column>
							...
							<el-dialog title="物资明细" :visible.sync="dialogTableVisible" width=98%>
								...
								<div class="ztable">
									<el-table :data="item1.tableDatas" ref="table" tooltip-effect="dark" border stripe style="width: 100%">
...
<script>
	...
	methods: {
		deleteProject(row){
			this.ProjetAeeay=_.without(this.ProjetAeeay,row);
		},
		...
		accessoryaddRow(item, item1) {
			var list = {
				rowNum: '',
				fileNam: item1.fileName,
				annexChinaName: item1.annexChinaName,
		...
		spsave(item, item1) {
			...
			item1.tableDatas = item1.cktableData
		},
		// 在线编辑
		editings(row) {
		...
		var param = {
			...
			startWorkflowParameterVo: {
				...
				name: this.name,
				processName: "年度科研计划项目",
				userId: sessionStorage.userId
			},
			submitType: m
		}
			postAjaxx("/api/planmanagementService/annualResearchPlan/saveTechnologyPlan", JSON.stringify(param)).then(res => {
				if(res.code == 0) {
					this.$message("成功");
					this.$router.push({
						path: '/home/index',
	...
	created() {
		...
		postAjaxJson(`/api/process-service/userTask/getFirstExaminePerson/ndkyxmjh`, param).then(res => {
			console.log(res)
			if(res.code == 0) {
