					...
					<el-tabs v-model="active" style="">
						<el-tab-pane name="0">
							...
							<el-table :data="findAllPlanApplyList" ref="findAllPlanApplyList" @select="selectMemoryFn" @select-all="selectAll" @selection-change="handleSelectionChange" tooltip-effect="dark" border stripe style="width: 100%">
								...
								<el-table-column prop="planType" label="计划名称" />
						...
						<el-tab-pane name="1">
							...
							<div>
								<el-tabs v-model="activeName" @tab-click="handleClick">
									<el-tab-pane v-for="(item, index) in editableTabs" :key="item.planType" :label="item.planType" :name="item.planType">
										<el-table :data="item.planBudgetprojectList" @selection-change="handleSelectionChange1" ref="tableDatas" tooltip-effect="dark" border stripe style="width: 100%">
											...
											<el-table-column label="操作">
												<template slot-scope="scope">
													<el-button type="text" size="small" @click="updateGoods(scope.row,scope.index,index)">编辑商品</el-button>
												</template>
											</el-table-column>
										</el-table>
							...
							<div class="content">
								...
								<div class="essential_top">
									...
									<el-tabs v-model="activeName1">
										<el-tab-pane v-for="(item, index) in endTable" :key="item.planType" :label="item.planType" :name="item.planType">
		...
		methods: {
			...
			handleSelectionChange1(val) {
				...
				_.forEach(this.editableTabs, value => {
					_.forEach(this.multipleSelection2[value.planType], (v, key) => {
			...
			planList() {
				...
				getAjax("/api/planmanagementService/annualSpecialFundSummaryPlan/findPagePlanCollect", param).then(data => {
					if(data.code == 0) {
			...
			handleClick(tab, event) {
				...
				this.activeName = this.editableTabs[tab.index].planType;
				this.planMoney = 0;
			...
			next() {
				...
				this.stepActive = 1;
				var param = {
					planCollectIds: String(this.multipleSelection.map(val => {
						return val.id
					}))
				}
				getAjax("/api/planmanagementService/annualSpecialFundSummaryPlan/findPlanCollectByPlanCollectId", param).then(data => {
					if(data.code == 0) {
						this.editableTabs = data.data;
						//						this.getSpanArr(data.data[0].planBudgetProjectList);
						this.activeName = data.data[0].planType;
						this.planMoney = 0;
			...
			getGoodsList() {
				...
				getAjax("/api/planmanagementService/annualSpecialFundSummaryPlan/findProjectGoodsByBudgetProjectId", param).then(data => {
					if(data.code == 0) {
			...
			save() {
				...
				this.updateGoodsObj.goodsList = this.selectlistRow;
				this.zxfbjsp = false;
			},
			nexts() {
				this.$refs['dynamicValidateForm'].validate((valid) => {
					if(valid) {
						...
						this.endTable = _.uniq(this.endTable, "planType");
						//						this.getSpanArr(this.endTable[0].planBudgetProjectList);
						this.activeName1 = this.endTable[0].planType;
						this.planMoney = 0;
			...
			submit(m) {
				...
				var planCollectProjectVoList = [];
				var planIds = [];
				planIds=_.map(this.multipleSelection,val=>{return val.id});
...
//						planIds.push(val.budget_id);
				...
				var param = {
					businessForm: "/special/specialsh",
					deploymentKey: "ndzxzjtblc",
					processName: this.formName,
					planType: this.formName,
					fromId:this.$route.query.id,
					collectIds: String(planIds),
					...
					planCollectProjectVoList: planCollectProjectVoList,
					summaryName: this.dynamicValidateForm.usename,
					submitType:m,
				}
				postAjaxJson("/api/planmanagementService/annualSpecialFundSummaryPlan/save", param).then(res => {
					if(res.code == 0) {
						this.$message("成功");
						this.$router.push("/home/index");
					} else {
		...
		created() {
			var name = this.$route.query.formName == "年度专项折旧非生产设备资金汇总计划" ? "ndzxzjfscsbhzlc" : "ndzxaqhz"
			postAjaxJson(`/api/process-service/userTask/getFirstExaminePerson/` + name).then(res => {
				if(res.code == 0) {
