...
<script>
	...
	export default {
		...
		created() {
			getAjax("/api/planmanagementService/annualSpecialFundSummaryPlan/findPlanShByPlanCollectId?planCollectId=" + this.$route.query.id).then(res => {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						postAjax("/api/planmanagementService/annualSpecialFundSummaryPlan/commonComplete/" + this.$route.query.taskId, param).then(res => {
							if(res.code == 0) {
								this.$message("成功");
