<template>
	<div style="padding:20px;">
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>各中心年度专项资金计划流程</el-breadcrumb-item>
		</el-breadcrumb>
		<div class="zxfplan">
			<h3>{{planBudget.title}}</h3>
			<span>{{planBudget.orgName}}</span>
			<span>{{planBudget.createUserName}}</span>
			<span style="width:200px;">{{planBudget.createDate}}</span>
			<strong>查看流程图</strong>
			<div class="zxftable">
				<template>
						...
						<el-tab-pane label="计划单" name="first">
							...
							<div class="ztable">
								...
								<el-table :data="zxftableData" ref="table" slot="empty" border stripe>
									...
									<el-table-column label="项目等级" align="center" prop="projectDesc">
									</el-table-column>
				...
				<el-dialog title="项目文件" :visible.sync="dialogTableVisible" width=90%>
...
<script>
	...
	export default {
		...
		created() {
			...
			var param = {
				planBudgetId: this.$route.query.businessKey
			}
			getAjax("/api/planmanagementService/annualSpecialSafetyFundPlan/findPlanDetailsByPlanBudgetId", param).then(res => {
				this.zxftableData = res.data.projectList;
				this.planBudget = res.data.planBudget?res.data.planBudget:{};
				this.planBudget.createUserName = res.data.planBudget.createUserName;
				this.planBudget.title= res.data.planBudget.title?res.data.planBudget.title:"年度科技资金汇总";
			});
		},
		methods: {
			getResult(value) {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						postAjax("/api/planmanagementService/annualSpecialSafetyFundPlan/commonComplete/"+this.$route.query.taskId, param).then(res => {
							if(res.code == 0) {
								this.$message("成功");
