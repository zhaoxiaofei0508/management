  ...
  <div style="padding:20px;">
    ...
    <div>
      ...
      >
        ...
        <el-upload class="upload-demo" ref="upload" :file-list="fileList" name="articleImage" :headers="handleHeader"
            :on-exceed="handleExceed" :on-success="handleSuccess" action="http://61.134.63.231:9090/organizationService/image/uploadImg">
...
<script>
import { postAjaxJson, postAjaxx, getAjax } from "@/utils/ajaxRequest";
      ...
      this.$refs['ruleForm'].validate((valid) => {
        ...
        var obj = {
          ...
          "planAnnexFile": {
            ...
            "id": "",
            "mainTableId": this.$route.query.businessKey,
            "ordernum": 0,
            "updateDate": "",
            "updateUser": sessionStorage.userName,
            "useable": ""
          }
        }
        
        postAjaxx(
          "/api/planmanagementService/planSubpackage/uploadMetting",
          JSON.stringify(obj)
        ).then(res => {
          console.log(res);
        });
  ...
  created() {
    ...
    postAjaxJson(
      ...
      res.data.forEach(item => {
        if(item.name == "同意"){
          this.confirmation.push(item)
          name = "false"
        }else if(item.name == "不同意"){
          this.confirmation.push(item)
        }
      });
      if(name != "false"){
