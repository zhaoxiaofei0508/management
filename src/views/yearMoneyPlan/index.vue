<template>
	<div style="padding:20px;">
		...
		<el-dialog title="流程图" :visible.sync="showModal" width=80%>
			<img src="../../../static/img/ndkjzjjh.png">
		</el-dialog>
		<!-- 在线编辑 -->
...
<script>
	...
	methods: {
						...
						"useable": "1"
								...
								var param = {
									...
									startWorkflowParameterVo: {
										...
										businessForm: "/cashplan/cashplanT",
										departId: this.departId,
										deploymentKey: "ndkjzjxm",
										id: this.ruleForm.userId,
										name: this.name,
										processName: "年度科技资金项目",
										userId: sessionStorage.userId
									},
									submitType: m
								}
								postAjaxx("/api/planmanagementService/technologyPlan/saveTechnologyPlan", JSON.stringify(param)).then(res => {
									if(res.code == 0) {
										this.$message("成功");
										this.$router.push({
											path: '/home/index',
		...
		created() {
			...
			postAjaxJson(`/api/process-service/userTask/getFirstExaminePerson/ndkjzjxm`, param).then(res => {
				if(res.code == 0) {
