							...
							<div>
								<el-tabs v-model="activeName" @tab-click="handleClick">
									<el-tab-pane v-for="(item, index) in editableTabs" :key="item.orgName" :label="item.orgName" :name="item.orgName">
										<el-table :data="item.planBudgetProjectList" @selection-change="handleSelectionChange1" :span-method="arraySpanMethod" ref="tableDatas" tooltip-effect="dark" border stripe style="width: 100%">
	...
	export default {
		...
		methods: {
			...
			submit0() {
				...
				var param = {
					businessForm: this.formName == "年度专项折旧非生产设备资金汇总计划" ? "/summary/summarys" : "/summaryReview/safetyChargeReview",
					deploymentKey: this.formName == "年度专项折旧非生产设备资金汇总计划" ? "ndzxzjfscsbhzlc" : "ndzxaqhz",
			...
			submit1() {
				...
				var param = {
					...
					nextAssigneeUserId: this.temp.userId,
					planProjectVo:planProjectVo,
					planIds: String(this.multipleSelection.map(val => {
						return val.id
					})),
					summaryName: this.dynamicValidateForm.usename,
				}
