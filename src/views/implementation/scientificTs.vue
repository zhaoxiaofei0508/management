  ...
  created() {
    ...
    postAjaxJson(
      `/api/processService/userTask/getNewTaskPathAndExaminePerson/${
        ...
        console.log(this.confirmation[0].nextActiveName)
        if(this.confirmation[0].nextActiveName == "填报人导入会议纪要") {
            this.confirmation.splice(0,1);
            this.confirmation.push(result2)
        }
        if(this.confirmation[0].nextActiveName == "结束") {
            this.confirmation.splice(0,1);
        }
    });
    getAjax(
