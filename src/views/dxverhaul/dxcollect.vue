					...
					<el-tabs v-model="active" style="">
						<el-tab-pane name="0">
							...
							<el-table :data="findAllPlanApplyList" ref="findAllPlanApplyList" @select="selectMemoryFn" @select-all="selectAll" @selection-change="handleSelectionChange" tooltip-effect="dark" border stripe style="width: 100%">
								...
								<el-table-column label="提报人" prop="createUserName" />
						...
						<el-tab-pane name="1">
							...
							<div>
								<el-table :data="jhhzdtableData" ref="tableDatas" tooltip-effect="dark" @selection-change="handleSelectionChange1" border stripe>
									<el-table-column type="selection" width="45" align="center"></el-table-column>
									<el-table-column prop="projectNumber" label="项目编码" width="120">
										<template slot-scope="scope">
									...
									<el-table-column label="项目预估金额" prop="projectMoney" width="110">
										<template slot-scope="scope">
											<el-input v-model="scope.row.projectMoney" @change="getMoney" class='inputss'></el-input>
										</template>
									</el-table-column>
									<el-table-column label="开始时间" prop="startDate">
										<template slot-scope="scope">
											<el-date-picker style="width:100%;margin-left:0px;" v-model="scope.row.startTime" type="date" placeholder="选择开始时间" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd">
											</el-date-picker>
										</template>
									</el-table-column>
									<el-table-column label="结束时间" prop="endDate">
										<template slot-scope="scope">
											<el-date-picker style="width:100%;margin-left:0px;" v-model="scope.row.endDate" type="date" placeholder="选择开始时间" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd">
											</el-date-picker>
										</template>
									</el-table-column>
									...
									<el-table-column label="是否核准制" prop="approval" width="120">
										<template slot-scope="scope">
											<el-select v-model="scope.row.approval">
												<el-option v-for="item in options" :key="item.zxftypevalue" :label="item.zxftypevalue" :value="item.zxftypevalue">
												</el-option>
											</el-select>
										</template>
									</el-table-column>
						...
						<el-tab-pane name="2">
							<div class="content">
								...
								<div class="essential_top">
									...
									<el-table :data="multipleSelection3" ref="tableDatas" tooltip-effect="dark" border stripe style="fontSize:12px">
										<el-table-column prop="projectNumber" label="项目编码" width="100" />
										<el-table-column prop="projectType" label="项目类别" width="100" />
										...
										<el-table-column label="附件" prop="isFile" />
										<el-table-column label="是否核准制" prop="approval" width="90" />
										<el-table-column label="提报部门" prop="orgName" />
	...
	export default {
		...
		data() {
			return {
				...
				zxftypevalue: '', //是否核制
				xztableData: [], //选择计划表格	
				jhhzdtableData: [], //汇总单表格
				tableDatas: [], //计划名称表格		
				yltableData: [], //预览表格
				tableDatass: [], //选择计划查看遮罩内容查看商品里层表格
				sptable: [], //选择计划查看商品表格
		...
		methods: {
			...
			planList() {
				...
				getAjax("/api/planmanagementService/annualSpecialFundPlan/findPageAllPlanBudgetList", param).then(data => {
					if(data.code == 0) {
			...
			submit0() {
				...
				_.forEach(this.multipleSelection3, val => {
					...
					planProjectVo.push({
						...
						planCollectGoodsList: val.goodsList
					});
				});
				planIds = _.uniq(planIds);
				var param = {
					businessForm: "/dxverhaul/dxcollects",
					deploymentKey: "nddxjhhzlc",
					processName: this.formName,
					flowId: this.$route.query.id,
					collectId: "",
					nextAssigneeUserId: this.temp.userId,
					planProjectVo: planProjectVo,
					planIds: String(planIds),
					summaryName: this.dynamicValidateForm.usename,
				}
				postAjaxJson("/api/planmanagementService/overhaulSummary/startOverhaulSummaryFlowDraft", param).then(res => {
					if(res.code == 0) {
						this.$message("成功");
						this.$router.push("/home/index");
					} else {
			...
			submit1() {
				...
				postAjaxJson("/api/planmanagementService/overhaulSummary/startOverhaulSummaryFlow", param).then(res => {
					if(res.code == 0) {
						this.$message("成功");
						this.$router.push("/home/index");
					} else {
			...
			getMoney() {
				this.planMoney = 0;
				_.forEach(this.jhhzdtableData, val => {
			...
			next() {
				...
				getAjax("/api/planmanagementService/annualSpecialFundPlan/findDetailsByPlanBudgetProjectList", param).then(data => {
					if(data.code == 0) {
						this.jhhzdtableData = data.data;
						this.planMoney = 0;
						_.forEach(this.jhhzdtableData, val => {
							this.planMoney += Number(val.projectMoney);
						});
